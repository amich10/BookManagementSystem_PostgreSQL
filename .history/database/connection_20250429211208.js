...
db.users = require('./models/user.model')(sequelizeObj,DataTypes)
db.products = require('./models/product.model')(sequelizeObj,DataTypes)

//migration
sequelizeObj.sync({force:false, alter:false}) //force:true => migrates any changes made like change made in column name but this is not done as this deletes all data when changes made
// alter:true => only change the colum name
//migrate should only de done when all changes are finished otherwise it should be kept false
.then(() =>{
